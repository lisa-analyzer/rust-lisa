 ...
 * {@value OpenCallPolicy#RETURNED_VARIABLE_NAME} assigned to top.
 * This variable, that is also stored as computed expression, represent the
 * unknown result of the call.
 * 
...
public class RustReturnTopPolicy implements OpenCallPolicy {
	...
	public <A extends AbstractState<A, H, V, T>,
		...
		Optional<CompilationUnit> externCompilationUnit = call.getProgram().getUnits().stream()
				...
				.flatMap(unit -> ((CompilationUnit) unit).getImmediateAncestors().stream()) // get every the ancestors of the units
		...
		if (externCompilationUnit.isPresent()) {
			...
			if (externCodeMembers.size() == 0)
				throw new SemanticException("Found more than one function that matches the declaration of " + call + " in extern module");
			
			// There is no overloading in Rust, so there should be only one extern function with a specific name
			CodeMember externFn = externCodeMembers.stream().findFirst().get();
			
			PushAny pushAny = new PushAny(externFn.getDescriptor().getReturnType(), call.getLocation());
			Variable var = new Variable(externFn.getDescriptor().getReturnType(), RETURNED_VARIABLE_NAME, call.getLocation());
			return entryState.assign(var, pushAny, call);
		}
		
		throw new SemanticException("Unable to find implemented or extern call " + call);
	}
}
