 ...
 */
public class RustGreaterExpression extends BinaryExpression {
	 ...
	 */
	public RustGreaterExpression(CFG cfg, CodeLocation location,
				...
				if (leftType.canBeAssignedTo(correctType) && correctType.canBeAssignedTo(leftType))
					result = result
							.lub(state.smallStepSemantics(new it.unive.lisa.symbolic.value.BinaryExpression(leftType,
									left, right, ComparisonGt.INSTANCE, getLocation()), this));
			}
		return result;
	}
